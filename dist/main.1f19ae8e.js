...
var diffAttrs = function diffAttrs(oldAttr, newAttrs) {
  var patches = []; // add new attributes
  ...
  for (var _i2 = 0, _Object$entries2 = Object.entries(oldAttr); _i2 < _Object$entries2.length; _i2++) {
    _loop2();
  }
...
var diffChildren = function diffChildren(oldVChildren, newVChildren) {
  ...
  var _iterator2 = _createForOfIteratorHelper(zip(oldVChildren, newVChildren)),
      _step2;
  ...
  var _iterator3 = _createForOfIteratorHelper(newVChildren.slice(oldVChildren.length)),
      _step3;

  try {
    var _loop3 = function _loop3() {
      var additionalVPath = _step3.value;
      additionalPatches.push(function ($node) {
        $node.appendChild((0, _render.default)(additionalVPath));
        return $node;
      });
    };
  ...
  return function ($parent) {
    var _iterator4 = _createForOfIteratorHelper(zip(childPatches, $parent.childNodes)),
        _step4;
    ...
    var _iterator5 = _createForOfIteratorHelper(additionalPatches),
        _step5;

    try {
      for (_iterator5.s(); !(_step5 = _iterator5.n()).done;) {
        var patch = _step5.value;
        patch($parent);
      }
    } catch (err) {
      _iterator5.e(err);
...
var diff = function diff(vOldNode, vNewNode) {
  ...
  var pathAttrs = diffAttrs(vOldNode.attrs, vNewNode.attrs);
  var pathChildren = diffChildren(vOldNode.children, vNewNode.children);
  return function ($node) {
    pathAttrs($node);
    pathChildren($node);
    return $node;
